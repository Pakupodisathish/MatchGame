...
class App extends Component {
  ...
  onClickImageItem = id => {
    const {isMatchedId} = this.state
    const matchedList = imagesList.filter(
      eachImageItem => eachImageItem.id !== isMatchedId,
    )
    const imageItem =
      matchedList[Math.ceil(Math.random() * matchedList.length - 1)]
    const isMatched = isMatchedId === id
    if (isMatched) {
      this.setState(prevState => ({
        score: prevState.score + 1,
        isMatched,
        imageToBeMatched: imageItem.imageUrl,
        isMatchedId: imageItem.id,
      }))
    }
    clearInterval(this.timerId)
    this.setState({
  ...
  filteringImageListWithCategory = () => {
  ...
  render() {
    const filteredImagesList = this.filteringImageListWithCategory()
    const {
      activeTabId,
      imageToBeMatched,
      score,
      ...
      <div>
        <div className="navbar">
          ...
          <ul className="navitems">
            ...
            <li className="timer-logo-container">
              <img
        ...
        <div className="game-bg-container">
          {isMatched ? (
            ...
            <div className="game-score-card-container">
              <img
                src="https://assets.ccbp.in/frontend/react-js/match-game-trophy.png"
                className="trophy-image"
                alt="trophy"
              />
              <p className="result-score-text">YOUR SCORE</p>
              <p className="result-score-text">{score}</p>
              <button
                className="play-again-btn-container"
                type="button"
